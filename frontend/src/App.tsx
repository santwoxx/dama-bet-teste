...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-stone-100 flex flex-col font-sans select-none antialiased selection:bg-amber-500/30 selection:text-white relative">
      ...
      {/* Banner Topo (Só no Lobby) — grudado no topo da tela, imagem cruzando de um lado a outro em loop */}
      {currentView !== 'game' && (
        <>
          <div className="fixed top-0 inset-x-0 z-40 h-16 sm:h-24 md:h-28 overflow-hidden bg-[#0a0a0a] border-b-2 border-[#FABF18]/40 shadow-[0_4px_25px_rgba(250,191,24,0.25)]">
            <motion.img
          ...
          {/* Spacer so the fixed banner doesn't cover the header/content below it */}
      ...
      {/* Banner Fim de Página — grudado embaixo da tela, imagem cruzando no sentido oposto ao do topo */}
      {currentView !== 'game' && (
        <div className="fixed bottom-0 inset-x-0 z-40 h-16 sm:h-24 md:h-28 overflow-hidden bg-[#0a0a0a] border-t-2 border-[#FABF18]/40 shadow-[0_-4px_25px_rgba(250,191,24,0.2)]">
          <motion.img
      ...
      {/* Spacer so the fixed bottom banner never covers footer content at max scroll */}
      {currentView !== 'game' && <div className="h-16 sm:h-24 md:h-28 w-full bg-[#1C120D]" aria-hidden="true" />}
      ...
      <a 
        ...
        className={`fixed right-6 z-[100] flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20bd5a] text-white px-3 sm:px-4 py-2 sm:py-3 rounded-full shadow-[0_4px_20px_rgba(37,211,102,0.4)] transition-all hover:scale-105 hover:-translate-y-1 group border border-white/20 ${currentView !== 'game' ? 'bottom-20 sm:bottom-28 md:bottom-32' : 'bottom-6'}`}
      >
        <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 sm:w-6 sm:h-6 drop-shadow">
        ...
        <div className="flex flex-col items-start leading-none pt-0.5">
          <span className="font-black text-xs sm:text-sm drop-shadow-sm tracking-wide">Gerente VIP</span>
          <span className="text-[8px] sm:text-[9px] font-medium opacity-90 hidden sm:block tracking-wider uppercase">Parcerias</span>
        </div>
      </a>
